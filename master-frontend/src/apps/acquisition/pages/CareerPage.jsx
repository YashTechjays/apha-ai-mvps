...
import { acqCareerApi, acqLeadApi } from '../../../api/client'
...
export default function CareerPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    try {
      const res = await acqCareerApi.score({ ...form, session_id: sessionId })
      setResult(res.data)
      setStep(4)
    } catch {
  ...
  const handleLeadCapture = async (email, name) => {
    ...
    // Fetch action plan
    try {
      const planRes = await acqCareerApi.getActionPlan(result.usage_id, sessionId)
