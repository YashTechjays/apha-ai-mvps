...
Scores across 6 competency dimensions + generates action plan.
"""
import json
...
from backend.ai.prompts import CAREER_SCORE_PROMPT, CAREER_ACTION_PLAN_PROMPT
...
def score_career(profile: dict) -> dict:
    """Score career readiness from a completed assessment profile."""
    profile_json = json.dumps(profile, indent=2)

    prompt = CAREER_SCORE_PROMPT.format(profile_json=profile_json)
    client = _get_client()
    ...
    for dim_id, score in scores.get("scores", {}).items():
        enriched_scores[dim_id] = {
            ...
            "apha_resources": next(
                (
                    d["apha_resources"]
                    for d in _competencies["dimensions"]
                    if d["id"] == dim_id
                ),
                [],
            ),
    ...
    # Peer comparison by career stage
    career_stage = profile.get("career_stage", "mid_career")
    expected = _competencies["career_stages"].get(career_stage, {"expected_score": 60})[
        "expected_score"
    ]
...
def generate_action_plan(profile: dict, scores: dict) -> dict:
    """Generate locked action plan (revealed after lead capture)."""
