...
Computes salary benchmark + generates personalized AI commentary.
"""
...
from backend.ai.prompts import SALARY_ANALYSIS_PROMPT, SALARY_QUICK_INSIGHT_PROMPT
...
def compute_salary_benchmark(
    ...
    """Compute salary percentiles for a given profile."""
    ...
    if specialty_data:
        p25 = specialty_data["p25"]
        p50 = specialty_data["p50"]
        p75 = specialty_data["p75"]
        p90 = specialty_data["p90"]
    else:
    ...
    p25 = int(p25 * exp_mult)
    p50 = int(p50 * exp_mult)
    p75 = int(p75 * exp_mult)
    p90 = int(p90 * exp_mult)
    ...
    return {
        ...
        "member_premium_salary": member_premium_salary,
...
def generate_salary_analysis(
    ...
    """Full salary benchmark + AI commentary."""
    benchmark = compute_salary_benchmark(
        state, specialty, license_type, years_experience, current_salary
    )
