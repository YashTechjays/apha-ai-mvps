...
from apps.acquisition.api.schemas.career import CareerProfile, CareerScoreResponse
from apps.acquisition.ai.career_engine import score_career, generate_action_plan
from apps.acquisition.rate_limiter.redis_limiter import check_limit
from core.logger import get_logger

router = APIRouter(prefix="/career", tags=["acquisition-career"])
...
def get_action_plan(usage_id: str, session_id: str, db: Session = Depends(get_db)):
    ...
    lead = db.query(AcquisitionLead).filter(AcquisitionLead.session_id == session_id).first()
    if not lead:
        raise HTTPException(
