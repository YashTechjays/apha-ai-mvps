...
TOOL_LIMITS = {
    "salary": settings.acq_salary_free_checks_per_day,
    "interaction": settings.acq_interaction_free_checks_per_day,
    "career": settings.acq_career_free_assessments_per_day,
}
...
try:
    ...
    logger.warning("Redis unavailable - rate limiting disabled for acquisition funnels")
...
def check_limit(ip: str, tool: str) -> tuple[bool, dict]:
    ...
    key = f"rl:acq:{tool}:{_hash_ip(ip)}:{today}"
